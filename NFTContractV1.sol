...
contract TestContract is ERC721, Ownable {
    ...
    mapping(uint256 => listing[]) history;
    mapping(uint256 => listing) traded;

    listing [] dailyTraded;
    uint256 [] listed;
    function addTraded(uint256 _id, address _account, uint256 _value, uint256 _start, uint256 _stop) internal {
        if(dailyTraded.length > 0)
        if(dailyTraded[dailyTraded.length.length-1].stop < _stop - 86400)
        dailyTraded = new listing[];
        dailyTraded.push(listing(_id, _account, _value, _start, _stop));
    }
    function listTrade(uint256 _id, uint256 _value) public {
        require(msg.sender == ownerOf(_id), "Not NFT owner");
        require(history[_id].[history[_id].length-1].stop == 1, "NFT already listed");
        history[_id].push(listing(_id, _account, _value, block.timestamp, 1));
    }
    ...
    function buyTrade(uint256 _id) public payable {
        require(msg.sender != ownerOf(_id), "Already NFT owner");
        require(history[_id].[history[_id].length-1].stop == 1, "NFT not listed");
        ...
        uint256 _fee = _ammount * 5 / 100;
        payable(owner()).send(_fee));
        payable(_account).send(_amount - _fee));

        history[_id].[history[_id].length-1].stop = block.timestamp;
        traded.push(listing(_id, _account, _value, history[_id].[history[_id].length-1].start, _stop));
        volume++;
        tradeVolume += _value;
    }


    constructor(string memory baseURI) ERC721("Test", "TestNFT") public {
        setBaseURI(baseURI);
        for(uint i=0; i<150; i++) { _safeMint(owner(), i); }
        }
    ...
    function mintNFT(uint256 numOf) public payable {
        ...
        require(totalSupply().add(numOf) <= maxSupply);
        require(msg.value >= calculatePrice().mul(numOf), "MTV below mint price");

        for (uint i = totalSupply(); i < numOf; i++) {
    ...
    function calculatePriceMultiple(uint256 _amount) internal pure returns (uint256) {
        uint currentSupply = totalSupply();
    ...
    function mintMultipleNFT(uint256 _amount) public payable {
        ...
        require(msg.value >= calculatePrice().mul(_amount), "BNB value sent is below the price");
    ...
    function reserveGiveaway(uint256 _amount) public onlyOwner {
        ...
        uint256 index;
        for (index = 0; index < _amount; index++) {  _safeMint(owner(), currentSupply + index); }
        }
